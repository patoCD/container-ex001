...
# Logging
# -------------------------
def setup_logging(verbose: bool, dirname = "logs"):
    level = logging.DEBUG if verbose else logging.INFO

    BASE_DIR = ROOT
    log_dir = BASE_DIR / dirname
...
def transform_one(item):
    # Simulate per-record transformation (can fail independently)
    if item["value"] == 20:
        raise ValueError("Simulated transform error")

    return {
        **item,
        "value_x2": item["value"] * 2
    }
...
def find_file(filename: str) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(
            f"No file named '{filename}' found under '{ROOT}'."
        )
